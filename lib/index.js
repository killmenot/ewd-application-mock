...
module.exports = {
  ...
  handlers: {
    ...
    quux: function (message, session, send, finished) {
      debug('message: %s', JSON.stringify(message));

      finished({
        type: message.type,
        text: 'This is message was handled by quux handler'
      });
    },
  },
  ...
  workerResponseHandlers: {
    ...
    quux: function (message) {
      debug('message: %s', JSON.stringify(message));

      this.io.toApplication({test: 'message2'}, 'ewd-application-mock');
